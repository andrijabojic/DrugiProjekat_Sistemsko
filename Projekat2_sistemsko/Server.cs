...
class Server
{
    ...
    private async Task ProcessRequestAsync()
    {
        ...
        else
        {
            ...
            lock (cityLock)
            {
                jsonResult = _cache.Get(city);

                if (jsonResult == null)
                {
                    Console.WriteLine($"Promasaj kesa! Zovem API za {city}...");
                    Task<string> fetchTask = FetchFromWeatherApiAsync(city);

                    Task processingTask = fetchTask.ContinueWith(prethodniTask =>
                    {
                        string rawJson = prethodniTask.Result;

                        if (rawJson != null)
                        {
                            string parsedJson = ParseJSON(rawJson);
                            _cache.Add(city, parsedJson);

                            SendResponse(context, parsedJson, HttpStatusCode.OK);
                        }
                        else
                        {
    ...
    private async Task<string> FetchFromWeatherApiAsync(string city)
    {
